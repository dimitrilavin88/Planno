...
-- UPDATED: Fixed duplicate email handling with ON CONFLICT
...
END IF;
-- Insert host participant using INSERT ... ON CONFLICT to handle edge cases
...
-- Create guest participant only if email is different from host (case-insensitive)
-- This prevents duplicate key violation when participant books with their own email
IF LOWER(TRIM(COALESCE(p_participant_email, ''))) != LOWER(TRIM(v_host_email))
AND TRIM(COALESCE(p_participant_email, '')) != '' THEN
INSERT INTO public.meeting_participants (
    meeting_id,
    name,
    email,
