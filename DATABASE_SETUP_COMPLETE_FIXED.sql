...
-- FIXED: Added SECURITY DEFINER and SET search_path, using helper function for email
...
-- Get host email using helper function
v_host_email := public.get_user_email(p_host_user_id);
...
VALUES (
        ...
        p_host_user_id,
        'Host',
        COALESCE(v_host_email, 'host@example.com'),
        true,
        'accepted'
    );
INSERT INTO public.meeting_participants (
...
EXCEPTION
WHEN OTHERS THEN IF p_lock_id IS NOT NULL THEN
DELETE FROM public.time_slot_locks
WHERE locked_by = p_lock_id;
END IF;
RETURN jsonb_build_object(
...
FROM public.group_event_type_hosts geth
    JOIN public.users u ON u.id = geth.user_id
WHERE geth.group_event_type_id = p_group_event_type_id LOOP
SELECT EXISTS (
        SELECT 1
        FROM public.meetings
        WHERE host_user_id = v_host.id
...
FROM public.group_event_type_hosts geth
    JOIN public.users u ON u.id = geth.user_id
WHERE geth.group_event_type_id = p_group_event_type_id LOOP -- Get host email using helper function
    v_host_email := public.get_user_email(v_host.id);
INSERT INTO public.meeting_participants (
        meeting_id,
        user_id,
        name,
